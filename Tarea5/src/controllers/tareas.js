...
class ControladorTareas{
    static listar(req, res){
        Tarea.find({}).lean()
            ...
            .catch(err => {
                res.status(404).send('No se lograron encontrar las tareas');
            });
    }
    static ver(req,res){
        Tarea.findById(req.params.id).then(res =>{
            res.status(200).send('Ver tarea' + req.params.id);
        }).catch(err =>{
            res.status(404).send('No pudo encontrar tarea');
        });
    }
    static postTarea(req,res){
        try{
            ...
            tarea.save().then(res=>{
                res.status(200).send('Se creo tarea correctamente');
            }).catch(err =>{
                res.status(400).send('datos no validos');
            });
        }catch (err){
            res.status(404).send('No se logro encontrar la tarea');
        }
    }
    ...
    static borrar(req,res){
        Tarea.findByIdAndDelete(req.params.id).then(tarea =>{
            ...
            res.status(200).send('Se elimino la tarea  correctamente id: ', req.params.id);
